...
const INFLAM = [
  ...
  { nome: 'Ferritina', und: 'ng/mL', rows: [
    { label: 'Pré-escolar', desc: '7 – 85',   cor: '#10B981', fundo: '#ECFDF5' },
    { label: 'Escolar',     desc: '10 – 120', cor: '#10B981', fundo: '#ECFDF5' },
    { label: 'Adol. M',     desc: '12 – 150', cor: '#10B981', fundo: '#ECFDF5' },
    { label: 'Adol. F',     desc: '12 – 120', cor: '#10B981', fundo: '#ECFDF5' },
...
const CORTISOL_ROWS = [
  ...
  { nivel: '3 – 18', desc: 'Zona cinzenta — interpretar com clínica; considerar teste de estímulo', cor: '#F59E0B', fundo: '#FFFBEB' },
  { nivel: '> 18',   desc: 'Praticamente exclui insuficiência adrenal em estresse',                  cor: '#10B981', fundo: '#ECFDF5' },
];
const HOR_CRESC = [
...
const HOMA_INTERP = [
  { nivel: '< 2,5',     desc: 'Normal',                               cor: '#10B981', fundo: '#ECFDF5' },
  { nivel: '2,5 – 3,4', desc: 'Resistência à insulina limítrofe',     cor: '#F59E0B', fundo: '#FFFBEB' },
  { nivel: '≥ 3,5',     desc: 'Resistência à insulina estabelecida',  cor: '#EF4444', fundo: '#FEF2F2' },
];

const GASTRO_CEL = [
  { nome: 'Anti-tTG IgA', tipo: 'Triagem 1ª linha', interp: [
    { nivel: 'Negativo', desc: 'Celíaca improvável em contexto adequado', cor: '#10B981', fundo: '#ECFDF5' },
    { nivel: '1× – < 10× LSN', desc: 'Borderline — repetir com IgA total; biópsia duodenal indicada', cor: '#F59E0B', fundo: '#FFFBEB' },
    { nivel: '≥ 10× LSN', desc: 'ESPGHAN 2020: diagnóstico possível sem biópsia se EMA+ + HLA DQ2/DQ8 + sintomas', cor: '#10B981', fundo: '#ECFDF5' },
  ...
  { nome: 'IgA total', tipo: 'Controle obrigatório', interp: [
    { nivel: '< 0,07 g/L', desc: 'Deficiência de IgA — anti-tTG IgA NÃO confiável', cor: '#EF4444', fundo: '#FEF2F2' },
    { nivel: '≥ 0,07 g/L', desc: 'IgA suficiente — anti-tTG IgA confiável', cor: '#10B981', fundo: '#ECFDF5' },
  ], alerta: null },
  { nome: 'Anti-EMA IgA', tipo: '2ª linha / confirmação', interp: [
    { nivel: 'Negativo', desc: 'Não confirma celíaca', cor: '#10B981', fundo: '#ECFDF5' },
    { nivel: 'Positivo', desc: 'Alta especificidade (> 98%) — forte evidência de celíaca ativa', cor: '#EF4444', fundo: '#FEF2F2' },
  ], alerta: null },
  { nome: 'HLA DQ2 / DQ8', tipo: 'Exclusão diagnóstica', interp: [
    { nivel: 'Negativo para ambos', desc: 'Praticamente exclui celíaca (VPN > 99%)', cor: '#10B981', fundo: '#ECFDF5' },
    { nivel: 'Positivo DQ2 ou DQ8', desc: 'Necessário mas não suficiente (30–40% da pop. geral)', cor: '#F59E0B', fundo: '#FFFBEB' },
  ], alerta: null },
];
const GASTRO_DII = [
  { nome: 'Calprotectina Fecal', und: 'μg/g', rows: [
    { nivel: '< 50',      desc: 'Normal — DII improvável',                              cor: '#10B981', fundo: '#ECFDF5' },
    { nivel: '50 – 200',  desc: 'Zona cinzenta — repetir em 4–6 semanas',              cor: '#F59E0B', fundo: '#FFFBEB' },
    { nivel: '200 – 600', desc: 'Elevada — sugestivo de DII ou infecção intestinal',    cor: '#F97316', fundo: '#FFF7ED' },
    { nivel: '> 600',     desc: 'Muito elevada — DII ativa; colonoscopia indicada',     cor: '#EF4444', fundo: '#FEF2F2' },
  ], alerta: 'Lactentes < 12 meses: valores normais podem ser > 200–300 μg/g fisiologicamente.' },
  { nome: 'ASCA IgG/IgA', und: 'qualitativo', rows: [
    { nivel: 'Positivo', desc: 'Associado a Doença de Crohn (sens. ~60%, espec. ~90%)', cor: '#F97316', fundo: '#FFF7ED' },
    { nivel: 'Negativo', desc: 'Não exclui Crohn', cor: '#10B981', fundo: '#ECFDF5' },
  ], alerta: null },
  { nome: 'p-ANCA', und: 'qualitativo', rows: [
    { nivel: 'Positivo', desc: 'Associado a Retocolite Ulcerativa (sens. ~65%, espec. ~90%)', cor: '#F97316', fundo: '#FFF7ED' },
    { nivel: 'Negativo', desc: 'Não exclui RCU', cor: '#10B981', fundo: '#ECFDF5' },
  ], alerta: null },
  { nome: 'H. pylori — Nota', und: '', rows: [
    { nivel: 'Indicado', desc: 'Teste respiratório (uréia marcada) ou antígeno nas fezes', cor: '#3B82F6', fundo: '#EFF6FF' },
    { nivel: 'NÃO indicado', desc: 'Sorologia IgG: baixa sens./espec. em pediatria (ESPGHAN 2016)', cor: '#EF4444', fundo: '#FEF2F2' },
  ], alerta: null },
];

const VITAM = [
  { nome: 'Vitamina D — 25(OH)D', und: 'ng/mL', rows: [
    { nivel: '< 20',     desc: 'Deficiência — suplementação terapêutica indicada', cor: '#EF4444', fundo: '#FEF2F2' },
    { nivel: '20 – 29',  desc: 'Insuficiência',                                    cor: '#F97316', fundo: '#FFF7ED' },
    { nivel: '30 – 100', desc: 'Suficiência',                                      cor: '#10B981', fundo: '#ECFDF5' },
    { nivel: '> 100',    desc: 'Potencial toxicidade — verificar suplementação',   cor: '#F59E0B', fundo: '#FFFBEB' },
  ...
  { nome: 'Vitamina B12 (Cobalamina)', und: 'pg/mL', rows: [
    { nivel: '< 200',     desc: 'Deficiência',                                    cor: '#EF4444', fundo: '#FEF2F2' },
    { nivel: '200 – 300', desc: 'Zona cinzenta — avaliar MMA urinário',           cor: '#F59E0B', fundo: '#FFFBEB' },
    { nivel: '> 300',     desc: 'Normal',                                          cor: '#10B981', fundo: '#ECFDF5' },
  ...
  { nome: 'Saturação de Transferrina', und: '%', rows: [
    { nivel: '< 16%', desc: 'Deficiência de ferro funcional',                cor: '#EF4444', fundo: '#FEF2F2' },
    { nivel: '16–50%',desc: 'Normal',                                         cor: '#10B981', fundo: '#ECFDF5' },
    { nivel: '> 50%', desc: 'Sobrecarga de ferro — investigar hemocromatose', cor: '#F97316', fundo: '#FFF7ED' },
  ], nota: 'Sat. Transferrina (%) = (Ferro sérico ÷ TIBC) × 100. TIBC normal: 250–400 μg/dL.', tipo: 'interp' },
  { nome: 'Zinco', und: 'μg/dL', rows: [
    { nivel: '< 60',     desc: 'Deficiência — risco de atraso de crescimento, imunodeficiência', cor: '#EF4444', fundo: '#FEF2F2' },
    { nivel: '60 – 110', desc: 'Normal (pediátrico)',                                              cor: '#10B981', fundo: '#ECFDF5' },
...
function NotaBox({ children }) {
  return (
    <div style={{ background: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: 8, padding: '8px 10px', marginTop: 8, display: 'flex', gap: 8 }}>
      <Info size={14} color="#3B82F6" style={{ flexShrink: 0, marginTop: 1 }} />
      <span style={{ fontSize: 11, color: '#1E3A8A', lineHeight: 1.5 }}>{children}</span>
    </div>
  );
}
function AlertBox({ children }) {
  return (
    <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, padding: '8px 10px', marginTop: 8, display: 'flex', gap: 8 }}>
      <AlertTriangle size={14} color="#EF4444" style={{ flexShrink: 0, marginTop: 1 }} />
      <span style={{ fontSize: 11, color: '#991B1B', lineHeight: 1.5 }}>{children}</span>
    </div>
  );
}
function VRow({ nome, und, valor, alerta, nota }) {
  return (
    <div style={{ background: '#fff', borderRadius: 10, padding: '10px 12px', marginBottom: 8, border: '1px solid #E5E7EB' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: '#1F2937' }}>{nome}</div>
          {und ? <div style={{ fontSize: 11, color: '#9CA3AF' }}>{und}</div> : null}
        </div>
  ...
  return (
    <div style={{ background: '#fff', borderBottom: '1px solid #E5E7EB', padding: '10px 12px', overflowX: 'auto' }}>
      <div style={{ display: 'flex', gap: 8, minWidth: 'max-content' }}>
        {cats.map(c => (
          <button key={c.id} onClick={() => onChange(c.id)} style={{ padding: '8px 14px', borderRadius: 20, fontSize: 13, fontFamily: 'DM Sans, sans-serif', border: `1.5px solid ${active === c.id ? CS : '#E5E7EB'}`, background: active === c.id ? CS : '#F9FAFB', color: active === c.id ? '#fff' : '#374151', fontWeight: active === c.id ? 700 : 400, cursor: 'pointer', whiteSpace: 'nowrap' }}>
            {c.label}
          </button>
        ))}
      </div>
  ...
  return (
    <div style={{ padding: '10px 12px 0', overflowX: 'auto' }}>
      <div style={{ display: 'flex', gap: 6, minWidth: 'max-content', marginBottom: 10 }}>
        {faixas.map((f, i) => (
          <button key={i} onClick={() => onChange(i)} style={{ padding: '6px 12px', borderRadius: 16, fontSize: 12, fontFamily: 'DM Sans, sans-serif', border: `1.5px solid ${active === i ? CD : '#E5E7EB'}`, background: active === i ? CL : '#F9FAFB', color: active === i ? CD : '#6B7280', fontWeight: active === i ? 700 : 400, cursor: 'pointer', whiteSpace: 'nowrap' }}>
            {f}
          </button>
        ))}
      </div>
    ...
    <div style={{ ...F, background: '#F8FAFC', minHeight: '100vh', paddingBottom: 80, maxWidth: 420, margin: '0 auto' }}>
        ...
        <div style={{ padding: '12px' }}>
          <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
            {['arterial','venosa'].map(m => (
              <button key={m} onClick={() => setGasMode(m)} style={{ flex: 1, padding: '10px', borderRadius: 10, fontSize: 13, fontWeight: gasMode === m ? 700 : 400, border: `2px solid ${gasMode === m ? CS : '#E5E7EB'}`, background: gasMode === m ? CL : '#F9FAFB', color: gasMode === m ? CD : '#374151', cursor: 'pointer', ...F }}>
                {m.charAt(0).toUpperCase() + m.slice(1)}
              </button>
            ))}
          </div>
          {(gasMode === 'arterial' ? GAS_A : GAS_V).map((g, i) => (
            <div key={i} style={{ background: '#fff', borderRadius: 10, padding: '10px 12px', marginBottom: 8, border: '1px solid #E5E7EB' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: '#1F2937' }}>{g.param}</div>
          ...
          {gasMode === 'arterial' && (
            <>
              <div style={{ fontSize: 11, fontWeight: 700, color: '#9CA3AF', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8, marginTop: 16 }}>Distúrbios ácido-base</div>
              {GAS_INTERP.map((d, i) => (
                <div key={i} style={{ background: '#fff', borderRadius: 10, padding: '10px 12px', marginBottom: 8, border: `1.5px solid ${d.cor}40` }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: d.cor, marginBottom: 4 }}>{d.dist}</div>
                  <div style={{ fontSize: 12, color: '#374151', marginBottom: 4 }}><strong>Critério:</strong> {d.crit}</div>
                  <div style={{ fontSize: 12, color: '#6B7280' }}><strong>Compensação:</strong> {d.comp}</div>
                </div>
              ))}
            </>
          )}
      ...
      {cat === 'bioquimica' && (
        <div style={{ padding: '12px' }}>
          ...
          {BIO.map((grupo, gi) => (
            <div key={gi}>
              <div style={{ fontSize: 11, fontWeight: 700, color: '#9CA3AF', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8, marginTop: 16 }}>{grupo.titulo}</div>
      ...
      {cat === 'inflamatorios' && (
        <div style={{ padding: '12px' }}>
          ...
          {INFLAM.map((item, ii) => (
            <div key={ii} style={{ background: '#fff', borderRadius: 12, padding: 14, border: '1px solid #E5E7EB', marginBottom: 10 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: '#1F2937', marginBottom: 2 }}>{item.nome}</div>
              <div style={{ fontSize: 11, color: '#9CA3AF', marginBottom: 10 }}>{item.und}</div>
              {item.rows.map((r, ri) => (
                <div key={ri} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 10px', borderRadius: 8, background: r.fundo, marginBottom: 5, gap: 8 }}>
                  ...
                  <span style={{ fontSize: 12, color: '#374151', textAlign: 'right', flex: 1 }}>{r.desc}</span>
                </div>
              ))}
        ...
        <div style={{ padding: '12px' }}>
          <div style={{ display: 'flex', gap: 6, marginBottom: 12, flexWrap: 'wrap' }}>
            {[{ id: 'puberdade', label: 'Puberdade / Sexuais' }, { id: 'tireoide', label: 'Tireoide' }, { id: 'cortisol', label: 'Cortisol / Adrenal' }, { id: 'crescimento', label: 'Crescimento / Insulina' }].map(s => (
              <button key={s.id} onClick={() => setHorSec(s.id)} style={{ padding: '7px 12px', borderRadius: 16, fontSize: 12, ...F, border: `1.5px solid ${horSec === s.id ? CS : '#E5E7EB'}`, background: horSec === s.id ? CS : '#F9FAFB', color: horSec === s.id ? '#fff' : '#374151', fontWeight: horSec === s.id ? 700 : 400, cursor: 'pointer', whiteSpace: 'nowrap' }}>
                {s.label}
              </button>
            ))}
          </div>
          ...
          {horSec === 'puberdade' && (
            <div>
              <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                {['F','M'].map(sx => (
                  <button key={sx} onClick={() => setHorSexo(sx)} style={{ flex: 1, padding: '10px', borderRadius: 10, fontSize: 14, fontWeight: horSexo === sx ? 800 : 400, border: `2px solid ${horSexo === sx ? CS : '#E5E7EB'}`, background: horSexo === sx ? CL : '#F9FAFB', color: horSexo === sx ? CD : '#374151', cursor: 'pointer', ...F }}>
                    {sx === 'F' ? '♀ Feminino' : '♂ Masculino'}
                  </button>
                ))}
              </div>
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#9CA3AF', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>Estadiamento de Tanner</div>
                <div style={{ display: 'flex', gap: 6 }}>
                  {HOR_TANNER_LABELS.map((t, i) => (
                    <button key={i} onClick={() => setTannerIdx(i)} style={{ flex: 1, padding: '8px 4px', borderRadius: 10, fontSize: 11, textAlign: 'center', fontWeight: tannerIdx === i ? 700 : 400, border: `1.5px solid ${tannerIdx === i ? CD : '#E5E7EB'}`, background: tannerIdx === i ? CL : '#F9FAFB', color: tannerIdx === i ? CD : '#6B7280', cursor: 'pointer', ...F }}>
                      {t}
                    </button>
                  ))}
                </div>
          ...
          {horSec === 'cortisol' && (
            <div>
              <div style={{ background: '#fff', borderRadius: 12, padding: 14, border: '1px solid #E5E7EB', marginBottom: 10 }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: '#1F2937', marginBottom: 4 }}>Cortisol matinal (8h) — μg/dL</div>
                <div style={{ fontSize: 11, color: '#9CA3AF', marginBottom: 10 }}>Coleta entre 7h–9h, em situação de estresse ou basal</div>
                {CORTISOL_ROWS.map((r, ri) => (
                  <div key={ri} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 10px', borderRadius: 8, background: r.fundo, marginBottom: 5, gap: 8 }}>
                    ...
                    <span style={{ fontSize: 12, color: '#374151', flex: 1, textAlign: 'right' }}>{r.desc}</span>
                  </div>
                ))}
            ...
            <div>
              {HOR_CRESC.map((h, i) => (
                <div key={i} style={{ background: '#fff', borderRadius: 12, padding: 14, border: '1px solid #E5E7EB', marginBottom: 10 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: '#1F2937', marginBottom: 2 }}>{h.nome}</div>
                  <div style={{ fontSize: 11, color: '#9CA3AF', marginBottom: 8 }}>{h.und}</div>
                  {h.rows.map((r, ri) => (
                    <div key={ri} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: ri < h.rows.length - 1 ? '1px solid #F3F4F6' : 'none' }}>
                      <span style={{ fontSize: 12, color: '#374151' }}>{r.nivel}</span>
                      <span style={{ fontSize: 13, fontWeight: 700, color: CS }}>{r.valor}</span>
                    </div>
                  ))}
                  {h.formula && (
                    <div style={{ background: '#F5F3FF', border: '1px solid #DDD6FE', borderRadius: 8, padding: '8px 10px', marginTop: 8 }}>
                      <div style={{ fontSize: 11, fontWeight: 700, color: '#5B21B6', marginBottom: 4 }}>Fórmula HOMA-IR</div>
                      <div style={{ fontSize: 12, color: '#374151', fontFamily: 'monospace' }}>{h.formula}</div>
                      <div style={{ marginTop: 8 }}>
                        {HOMA_INTERP.map((hm, hmi) => (
                          <div key={hmi} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 8px', borderRadius: 6, background: hm.fundo, marginBottom: 4, gap: 8 }}>
                            ...
                            <span style={{ fontSize: 12, color: '#374151' }}>{hm.desc}</span>
                          </div>
                        ))}
                      </div>
                    </div>
        ...
        <div style={{ padding: '12px' }}>
          <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
            {[{ id: 'celíaca', label: 'Doença Celíaca' }, { id: 'dii', label: 'DII / Inflamação' }].map(g => (
              <button key={g.id} onClick={() => setGastroDii(g.id)} style={{ flex: 1, padding: '10px', borderRadius: 10, fontSize: 13, fontWeight: gastroDii === g.id ? 700 : 400, border: `2px solid ${gastroDii === g.id ? CS : '#E5E7EB'}`, background: gastroDii === g.id ? CL : '#F9FAFB', color: gastroDii === g.id ? CD : '#374151', cursor: 'pointer', ...F }}>
                {g.label}
              </button>
            ))}
          </div>
          ...
          {gastroDii === 'celíaca' && (
            <div>
              ...
              {GASTRO_CEL.map((item, ii) => (
                <div key={ii} style={{ background: '#fff', borderRadius: 12, padding: 14, border: '1px solid #E5E7EB', marginBottom: 10 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: '#1F2937', marginBottom: 2 }}>{item.nome}</div>
                  <div style={{ fontSize: 11, color: '#9CA3AF', marginBottom: 10 }}>{item.tipo}</div>
                  {item.interp.map((r, ri) => (
                    <div key={ri} style={{ padding: '8px 10px', borderRadius: 8, background: r.fundo, marginBottom: 5 }}>
                      ...
                      <div style={{ fontSize: 12, color: '#374151', marginTop: 2 }}>{r.desc}</div>
                    </div>
                  ))}
                  {item.alerta && <AlertBox>{item.alerta}</AlertBox>}
                </div>
          ...
          {gastroDii === 'dii' && (
            <div>
              ...
              {GASTRO_DII.map((item, ii) => (
                <div key={ii} style={{ background: '#fff', borderRadius: 12, padding: 14, border: '1px solid #E5E7EB', marginBottom: 10 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: '#1F2937', marginBottom: 2 }}>{item.nome}</div>
                  {item.und && <div style={{ fontSize: 11, color: '#9CA3AF', marginBottom: 8 }}>{item.und}</div>}
                  {item.rows.map((r, ri) => (
                    <div key={ri} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 10px', borderRadius: 8, background: r.fundo, marginBottom: 5, gap: 8 }}>
      ...
      {cat === 'vitaminas' && (
        <div style={{ padding: '12px' }}>
          ...
          {VITAM.map((v, vi) => (
            <div key={vi} style={{ background: '#fff', borderRadius: 12, padding: 14, border: '1px solid #E5E7EB', marginBottom: 10 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: '#1F2937', marginBottom: 2 }}>{v.nome}</div>
              <div style={{ fontSize: 11, color: '#9CA3AF', marginBottom: 8 }}>{v.und}</div>
              {v.tipo === 'faixa'
                ? v.rows.map((r, ri) => (
                    <div key={ri} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: ri < v.rows.length - 1 ? '1px solid #F3F4F6' : 'none' }}>
                      <span style={{ fontSize: 12, color: '#374151' }}>{r.nivel}</span>
                      <span style={{ fontSize: 13, fontWeight: 700, color: CS }}>{r.valor}</span>
                    </div>
                  ))
