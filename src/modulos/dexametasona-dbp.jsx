    ...
    <div style={{ borderRadius:9, border:`1.5px solid ${cor}`, overflow:"hidden", marginBottom:8 }}>
      ...
      <div style={{ padding:"10px 14px" }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", paddingBottom:8, borderBottom:"1px dotted #E5E7EB", marginBottom:8 }}>
          <span style={{ fontSize:12, color:"var(--muted)" }}>Dose por tomada (12/12h)</span>
        ...
        {[
          ...
          <div key={conc} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"3px 0", fontSize:12 }}>
            <span style={{ color:"var(--muted)" }}>Vol [{conc}]{note && <span style={{ color:cor, fontWeight:700 }}>{note}</span>}</span>
            <span style={{ fontWeight:600, color: vol < 0.05 ? "#D97706" : "var(--text-2)" }}>
              {f3(vol)} mL{vol < 0.01 ? " ⚠" : ""}
            </span>
          </div>
        ))}
  ...
  return (
    <div style={{ background:"var(--bg)", borderRadius:10, padding:"12px 14px", marginBottom:12, border:"1px solid var(--border)" }}>
      ...
      <input type="text" inputMode="decimal" value={value} onChange={e=>set(e.target.value)} placeholder="Ex: 850"
        style={{ width:"100%", padding:"8px 10px", borderRadius:7, fontSize:14, border:`1.5px solid ${cor}50`, outline:"none", background:"var(--surface)", boxSizing:"border-box" }} />
...
function TabDART() {
  ...
  return (
    <div>
      ...
      {/* Indicação */}
      <div style={{ background:"var(--tint-blue)", borderRadius:10, padding:"12px 14px", marginBottom:10, border:"1px solid #BAE6FD" }}>
        <div style={{ fontSize:10, fontWeight:700, color:"var(--tx-blue)", letterSpacing:".08em", textTransform:"uppercase", marginBottom:8 }}>Indicação (DART)</div>
        {[
          ...
          <div key={i} style={{ display:"flex", gap:7, marginBottom:5, fontSize:12, color:"var(--text-2)" }}>
      ...
      {fases && (
        <>
          <p style={{ fontWeight:700, fontSize:13, color:"var(--text)", margin:"0 0 8px" }}>Esquema DART — 10 dias</p>
          ...
          <p style={{ fontSize:10, color:"var(--muted)", textAlign:"right", margin:"0 0 12px" }}>
          ...
          <button onClick={() => copiarTxt(gerarTxt("PROTOCOLO DART (Doyle 2006)", fases, pkKg, ACUM_DART, ["","Ref: Doyle LW et al. Pediatrics 2006;117:75-83"]), setCopied)}
            style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:7, width:"100%", padding:"11px 0", border:`1.5px solid ${C}`, borderRadius:10, background:"var(--surface)", color:C, fontSize:13, fontWeight:700, cursor:"pointer", marginBottom:14 }}>
      ...
      {/* Alertas */}
      <div style={{ background:"var(--tint-amber)", border:"1px solid #FDE68A", borderRadius:9, padding:"10px 14px", marginBottom:10, display:"flex", gap:8 }}>
        <AlertTriangle size={14} color="#D97706" style={{ flexShrink:0, marginTop:1 }} />
        <div style={{ fontSize:12, color:"var(--tx-amber)", lineHeight:1.5 }}>
      ...
      <div style={{ background:"var(--bg)", borderRadius:9, padding:"10px 14px", border:"1px solid var(--border)" }}>
        <div style={{ fontSize:10, fontWeight:700, color:"var(--muted)", textTransform:"uppercase", letterSpacing:".07em", marginBottom:8 }}>Monitorização</div>
        {["Glicemia a cada 6–12h nas primeiras 48h","Pressão arterial diária","Sinais de infecção (PCR, hemograma)","Avaliar desmame ventilatório após 48h"].map((m,i)=>(
          <div key={i} style={{ display:"flex", gap:7, marginBottom:4, fontSize:11.5, color:"var(--text-2)" }}>
...
function TabHMIB() {
  ...
  return (
    <div>
      ...
      <div style={{ background:"var(--tint-green)", borderRadius:10, padding:"12px 14px", marginBottom:10, border:"1px solid #BBF7D0" }}>
        <div style={{ fontSize:10, fontWeight:700, color:"var(--tx-green)", letterSpacing:".08em", textTransform:"uppercase", marginBottom:8 }}>Critérios de inclusão</div>
        {[
      ...
      <div style={{ background:"var(--tint-red)", borderRadius:9, padding:"9px 14px", marginBottom:10, border:"1px solid #FECACA", display:"flex", gap:8 }}>
        <AlertTriangle size={13} color="#DC2626" style={{ flexShrink:0, marginTop:2 }} />
        <span style={{ fontSize:12, color:"var(--tx-red)" }}><strong>Exclusão:</strong> tratamento de infecção ativa em curso.</span>
      </div>

      {/* BPD Estimator link */}
      <a href="https://neonatal.rti.org/bpdcalculator/" target="_blank" rel="noreferrer"
        style={{ display:"flex", alignItems:"center", gap:7, padding:"9px 14px", borderRadius:8, border:"1px solid #A7F3D0", background:"var(--tint-green)", marginBottom:14, textDecoration:"none" }}>
        <ExternalLink size={13} color={C} />
        <span style={{ fontSize:12, color:"var(--tx-green)", fontWeight:600 }}>BPD Outcome Estimator — NICHD (neonatal.rti.org)</span>
      </a>
      ...
      {fases && (
        <>
          <p style={{ fontWeight:700, fontSize:13, color:"var(--text)", margin:"0 0 8px" }}>Esquema HMIB — 9 dias</p>
          {fases.map(f => <FaseCard key={f.label} fase={f} cor={C} />)}
          ...
          <p style={{ fontSize:10, color:"var(--muted)", textAlign:"right", margin:"0 0 10px" }}>
            {ACUM_HMIB} mg/kg × {pkKg.toFixed(3)} kg
          </p>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:12 }}>
            {[["Intervalo entre ciclos","14 dias"],["Máx. ciclos","3 ciclos"]].map(([lbl,val])=>(
              <div key={lbl} style={{ background:"var(--bg)", borderRadius:8, padding:"8px 12px", border:"1px solid var(--border)", textAlign:"center" }}>
                <div style={{ fontSize:10, color:"var(--muted)", marginBottom:3 }}>{lbl}</div>
                <div style={{ fontSize:14, fontWeight:800, color:"var(--text-2)" }}>{val}</div>
              </div>
            ))}
          </div>
      ...
      <div style={{ background:"var(--bg)", borderRadius:10, padding:"12px 14px", border:"1px solid var(--border)" }}>
        <div style={{ fontSize:10, fontWeight:700, color:"var(--muted)", textTransform:"uppercase", letterSpacing:".07em", marginBottom:8 }}>Observações — Protocolo HMIB</div>
        {[
          ...
          <div key={i} style={{ display:"flex", gap:7, marginBottom:6, fontSize:12, color:"var(--text-2)" }}>
        ...
        {tabs.map((t,i)=>{
          ...
          return (
            <button key={i} onClick={()=>setTab(i)} style={{
              ...
              color:active?cores[i]:"var(--muted)", background:"transparent", border:"none",
