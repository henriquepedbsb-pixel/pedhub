...
const PROCEDIMENTOS = [
  { titulo: 'Laceração / Sutura complexa', nivel: 'Moderada', nivelCor: '#F97316', nivelFundo: '#FFF7ED', agentes: ['Cetamina IV/IM', 'Midazolam + Cetamina'], obs: 'Cetamina dissociativa — excelente para procedimentos dolorosos. Pré-medicação com midazolam reduz reações emergentes.', ambiente: 'PS · Pronto Atendimento' },
  { titulo: 'Redução de fratura', nivel: 'Moderada–Profunda', nivelCor: '#EF4444', nivelFundo: '#FEF2F2', agentes: ['Cetamina IV', 'Cetamina + Midazolam'], obs: 'Associar analgesia local (bloqueio de nervo) quando possível. Monitorização contínua obrigatória.', ambiente: 'PS · Sala de emergência' },
  { titulo: 'Drenagem de abscesso', nivel: 'Moderada', nivelCor: '#F97316', nivelFundo: '#FFF7ED', agentes: ['Cetamina IV/IM', 'Midazolam + Fentanil'], obs: 'Cetamina preferida pela manutenção de reflexos protetores de via aérea e rapidez de ação.', ambiente: 'PS · Pronto Atendimento' },
  { titulo: 'Punção lombar / Mielograma', nivel: 'Moderada', nivelCor: '#F97316', nivelFundo: '#FFF7ED', agentes: ['Midazolam + Fentanil', 'Cetamina IV/IM'], obs: 'Midazolam + Fentanil em procedimentos semi-eletivos. Cetamina em pacientes muito agitados ou não cooperativos.', ambiente: 'Ambulatório · Enfermaria' },
  { titulo: 'Exame de imagem (TC / RNM)', nivel: 'Leve–Moderada', nivelCor: '#F59E0B', nivelFundo: '#FFFBEB', agentes: ['Midazolam VO/IN', 'Óxido nitroso'], obs: 'Sedação mínima/moderada geralmente suficiente. Propofol para casos refratários — requer equipe especializada.', ambiente: 'Radiologia · Ambulatório' },
  { titulo: 'Acesso venoso difícil / PICC', nivel: 'Mínima', nivelCor: '#10B981', nivelFundo: '#ECFDF5', agentes: ['Midazolam VO/IN', 'Óxido nitroso'], obs: 'Associar EMLA tópico (aplicar 60 min antes) e técnicas não farmacológicas (distração, posicionamento).', ambiente: 'Enfermaria · Ambulatório' },
  { titulo: 'Endoscopia / Colonoscopia', nivel: 'Profunda', nivelCor: '#EF4444', nivelFundo: '#FEF2F2', agentes: ['Propofol + Fentanil'], obs: 'USO RESTRITO: ambiente de SO ou UTI com anestesiologista ou pediatra certificado em sedação profunda.', ambiente: 'Sala cirúrgica · UTI' },
];

const FARMACOS = [
  { id: 'midazolam', nome: 'Midazolam', classe: 'Benzodiazepínico', nivel: 'Mínima → Moderada', nivelCor: '#F59E0B', onset: 'VO: 15–30 min · IN: 5–10 min · IV: 2–3 min', duracao: '30–60 min', indicacoes: ['Ansiolítico pré-procedimento', 'Sedação leve/moderada', 'Pré-medicação para cetamina (reduz reações emergentes)'], contraindicacoes: ['Glaucoma de ângulo fechado', 'Apneia do sono grave não tratada', 'Hipersensibilidade a benzodiazepínicos'], monitoracao: ['SatO₂', 'FC', 'FR', 'Nível de consciência'], antagonista: 'Flumazenil', notas: ['Pode causar depressão respiratória dose-dependente — monitorar ventilação', 'Reação paradoxal (agitação) — mais comum em < 6 anos e em TEA/deficiência intelectual', 'Via intranasal (IN): útil sem acesso venoso — biodisponibilidade comparável ao IV'], ambiente: 'PS · Ambulatório · Enfermaria · UTI', cor: '#3B82F6', fundo: '#EFF6FF' },
    ...
    <div style={{ ...F, background: '#F8FAFC', minHeight: '100vh', paddingBottom: 80, maxWidth: 420, margin: '0 auto' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #E5E7EB', padding: '10px 12px', overflowX: 'auto' }}>
        <div style={{ display: 'flex', gap: 8, minWidth: 'max-content' }}>
          {ABAS.map(t => (
            <button key={t.id} onClick={() => setAba(t.id)} style={{ padding: '8px 14px', borderRadius: 20, fontSize: 13, ...F, border: `1.5px solid ${aba === t.id ? CI : '#E5E7EB'}`, background: aba === t.id ? CI : '#F9FAFB', color: aba === t.id ? '#fff' : '#374151', fontWeight: aba === t.id ? 700 : 400, cursor: 'pointer', whiteSpace: 'nowrap' }}>
              {t.label}
            </button>
          ))}
        </div>
          ...
          {FARMACOS.map((f) => {
            ...
            return (
              <div key={f.id} style={{ background: '#fff', borderRadius: 12, border: `1.5px solid ${aberto ? f.cor : '#E5E7EB'}`, marginBottom: 10, overflow: 'hidden' }}>
                <button onClick={() => toggle(f.id)} style={{ width: '100%', textAlign: 'left', padding: '14px', background: aberto ? f.fundo : '#fff', border: 'none', cursor: 'pointer', ...F, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ flex: 1 }}>
                  ...
                  {aberto ? <ChevronUp size={18} color={f.cor} style={{ flexShrink: 0, marginLeft: 8 }} /> : <ChevronDown size={18} color="#9CA3AF" style={{ flexShrink: 0, marginLeft: 8 }} />}
                </button>
                {aberto && (
                  <div style={{ padding: '0 14px 14px', borderTop: `1px solid ${f.cor}30` }}>
                    <div style={{ display: 'flex', gap: 10, marginTop: 12, marginBottom: 12 }}>
                      <div style={{ flex: 1, background: '#F9FAFB', borderRadius: 8, padding: '8px 12px' }}>
                        <div style={{ fontSize: 10, fontWeight: 700, color: '#9CA3AF', textTransform: 'uppercase' }}>Onset</div>
                        <div style={{ fontSize: 12, color: '#374151', marginTop: 2 }}>{f.onset}</div>
                      </div>
                      <div style={{ flex: 1, background: '#F9FAFB', borderRadius: 8, padding: '8px 12px' }}>
                        <div style={{ fontSize: 10, fontWeight: 700, color: '#9CA3AF', textTransform: 'uppercase' }}>Duração</div>
                    ...
                    <div style={{ marginBottom: 10 }}>
                      ...
                      {f.indicacoes.map((ind, ii) => <div key={ii} style={{ display: 'flex', gap: 8, alignItems: 'flex-start', padding: '3px 0' }}><div style={{ width: 6, height: 6, borderRadius: '50%', background: f.cor, flexShrink: 0, marginTop: 4 }} /><span style={{ fontSize: 12, color: '#374151' }}>{ind}</span></div>)}
                    </div>
                    <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, padding: '8px 10px', marginBottom: 10 }}>
                      ...
                      {f.contraindicacoes.map((c, ci) => <div key={ci} style={{ display: 'flex', gap: 8, alignItems: 'flex-start', padding: '2px 0' }}><div style={{ width: 6, height: 6, borderRadius: '50%', background: '#EF4444', flexShrink: 0, marginTop: 4 }} /><span style={{ fontSize: 12, color: '#7F1D1D' }}>{c}</span></div>)}
                    </div>
                    <div style={{ marginBottom: 10 }}>
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>{f.monitoracao.map((m, mi) => <span key={mi} style={{ background: '#F3F4F6', color: '#374151', fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 20 }}>{m}</span>)}</div>
                    </div>
                    <div style={{ background: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: 8, padding: '8px 10px', marginBottom: 10 }}>
                      ...
                      {f.notas.map((n, ni) => <div key={ni} style={{ display: 'flex', gap: 8, alignItems: 'flex-start', padding: '3px 0' }}><div style={{ width: 6, height: 6, borderRadius: '50%', background: '#3B82F6', flexShrink: 0, marginTop: 4 }} /><span style={{ fontSize: 12, color: '#1E3A8A', lineHeight: 1.5 }}>{n}</span></div>)}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#F9FAFB', borderRadius: 8, padding: '8px 10px' }}>
                      <Shield size={14} color="#6B7280" /><span style={{ fontSize: 12, color: '#374151' }}><strong>Antagonista:</strong> {f.antagonista}</span>
                    </div>
                  </div>
                )}
              </div>
        ...
        <div style={{ padding: '12px' }}>
          ...
          {NIVEIS.map((n, i) => (
            <div key={i} style={{ background: n.fundo, border: `1.5px solid ${n.borda}`, borderRadius: 12, padding: 16, marginBottom: 10 }}>
              ...
              <div style={{ fontSize: 12, color: '#374151', lineHeight: 1.6, marginBottom: 10, fontStyle: 'italic' }}>"{n.definicao}"</div>
      ...
      {aba === 'presedacao' && (
        <div style={{ padding: '12px' }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 14, border: '1px solid #E5E7EB', marginBottom: 12 }}>
            ...
            <div style={{ fontSize: 11, color: '#6B7280', marginBottom: 10, lineHeight: 1.5 }}>Aplicar o tempo mais restritivo compatível com o que foi ingerido por último.</div>
            {JEJUM.map((j, i) => (
          ...
          <div style={{ background: '#FFFBEB', border: '1.5px solid #FDE68A', borderRadius: 10, padding: 12, marginBottom: 12, display: 'flex', gap: 8 }}>
            ...
            <div style={{ fontSize: 12, color: '#92400E', lineHeight: 1.6 }}><strong>Emergência com risco de vida:</strong> Não retardar sedação aguardando jejum. Priorizar suporte de via aérea e ter aspirador pronto.</div>
          </div>
        </div>
      )}
            ...
            <div key={i} style={{ background: ant.fundo, border: `1.5px solid ${ant.borda}`, borderRadius: 12, padding: 16, marginBottom: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: '#6B7280', marginTop: 2 }}>Reverte: <strong>{ant.reverte}</strong></div>
                </div>
