...
function StepCard({ step, title, content, color }) {
  return (
    <div style={{ display: "flex", gap: 12, marginBottom: 10, padding: "10px 14px", borderRadius: 10, background: "var(--bg)", border: "1px solid var(--border)" }}>
      ...
      <div>
        <p style={{ fontWeight: 600, fontSize: 13, color: "var(--text)", margin: "0 0 3px" }}>{title}</p>
        <p style={{ fontSize: 12, color: "var(--text-2)", lineHeight: 1.5, margin: 0 }}>{content}</p>
      </div>
    </div>
  );
}
    ...
    <div style={{ borderRadius: 12, border: "1.5px solid " + cor, marginBottom: 16, overflow: "hidden" }}>
      ...
      <div style={{ padding: "12px 14px" }}>
        {criterios_baixo_risco && (
          <>
            <p style={{ fontWeight: 600, fontSize: 12, color: "var(--muted)", margin: "0 0 6px", letterSpacing: "0.04em" }}>CRITÉRIOS DE BAIXO RISCO (todos devem estar presentes)</p>
        ...
        {exames && (
          <>
            <p style={{ fontWeight: 600, fontSize: 12, color: "var(--muted)", margin: "10px 0 6px", letterSpacing: "0.04em" }}>EXAMES MÍNIMOS</p>
            {exames.map((e, i) => (
              <div key={i} style={{ display: "flex", gap: 6, marginBottom: 4 }}>
                <ChevronRight size={13} color={cor} style={{ flexShrink: 0, marginTop: 2 }} />
                <span style={{ fontSize: 12, color: "var(--text-2)" }}>{e}</span>
              </div>
            ))}
          </>
        )}
      ...
      <div style={{ padding: 16 }}>
        ...
        {tab === 3 && (
          <div>
            ...
            <p style={{ fontWeight: 700, color: "var(--text)", fontSize: 14, margin: "0 0 12px" }}>Critérios PECARN — baixo risco (todos devem estar presentes):</p>
            {[
              ...
              <div key={i} style={{ display: "flex", gap: 8, padding: "8px 14px", background: "var(--tint-green)", border: "1px solid #6EE7B7", borderRadius: 10, marginBottom: 6 }}>
